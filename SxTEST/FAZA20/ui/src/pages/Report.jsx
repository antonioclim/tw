...
const toExcelXML = (rows) => {
  const cells = (r) =>
    `<Cell><Data ss:Type='Number'>${r.lessonId}</Data></Cell>` +
...
const dl = (name, text, mime = "text/plain;charset=utf-8") => {
...
function aggregate(lessons, progress) {
  ...
  const rows = [...by.values()].map(r => ({
    ...r,
    avgScore: Math.round((r.scores.reduce((a, b) => a + b, 0) / r.scores.length) || 0)
  }));
...
export default function Report() {
  ...
  // demo progress; poți lega ulterior la un endpoint real (/api/progress)
  const [progress] = useState([
    { user: "u1", lessonId: 1, score: 80 },
    { user: "u1", lessonId: 2, score: 60 },
    { user: "u2", lessonId: 1, score: 90 }
  ]);
  ...
  // încarcă toate lecțiile din toate cursurile
  useEffect(() => {
    (async () => {
      try {
        const rc = await fetch("/api/courses");
        if (!rc.ok) throw 0;
        ...
        for (const c of courses) {
          ...
          if (r.ok) {
            ...
            for (const l of ls) res.push({ id: l.id, title: l.title, lang: l.lang, duration: l.duration });
          }
        }
        setLessons(res);
      } catch {
  ...
  const filtered = ag.rows.filter(
    (r) =>
      (!q || (r.title + " " + r.lang).toLowerCase().includes(q.toLowerCase())) &&
      (!lang || r.lang === lang) &&
      r.avgScore >= min
  );
  ...
  const pageRows = rows.slice((page - 1) * per, (page - 1) * per + per);

  // ------- export PDF (print-to-PDF cu fallback iframe) -------
  function exportPDF() {
    const tableHTML = document.getElementById("rptTbl")?.outerHTML || "";
    const style = `<style>
      body{font-family:system-ui;color:#111;margin:1rem}
    ...
    let w = null;
    try { w = window.open("", "_blank", "noopener,noreferrer,width=1024,height=768"); } catch {}
    if (w && w.document) {
      w.document.open(); w.document.write(html); w.document.close(); w.focus(); setTimeout(() => w.print(), 250); return;
    }
    const iframe = document.createElement("iframe");
    iframe.style.width = "0"; iframe.style.height = "0"; document.body.appendChild(iframe);
    const doc = iframe.contentDocument || iframe.contentWindow.document;
    doc.open(); doc.write(html); doc.close();
    setTimeout(() => { doc.defaultView.print(); document.body.removeChild(iframe); }, 300);
  }

  // ------- grafice SVG (fără librării) -------
  const donutSVG = (() => {
    ...
    for (const [lg, ct] of parts) {
      ...
      arcs += `<path d="M ${cx} ${cy} L ${x0} ${y0} A ${R} ${R} 0 ${large} 1 ${x1} ${y1} Z" fill="${colors[lg] || colors.other}" opacity="0.85"></path>`;
      start = end;
    }
  ...
  return (
    <div className="card">
      ...
      <div className="controls">
        ...
        <label>
          ...
          <input type="number" min="0" max="100" value={min}
                 onChange={(e) => { setMin(Number(e.target.value) || 0); setPage(1); }}
                 style={{ width: "5rem" }} />
        </label>
        <label>
          Per pagină:{" "}
      ...
      <table id="rptTbl">
        ...
        <tbody>
          {pageRows.map((r) => (
            <tr key={r.lessonId}><td>{r.title}</td><td>{r.lang}</td><td>{r.duration}</td><td>{r.avgScore}</td></tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="controls">
        <div className="chart" dangerouslySetInnerHTML={{ __html: donutSVG }} />
        <div className="chart" dangerouslySetInnerHTML={{ __html: stackedSVG }} />
      </div>
    </div>
  );
}
